...
(*try to change e:expr to a varlist and when calling make it a union of both expressions*)
 let print_set s = 
     VarSet.iter print_endline s
let rec renaming (x: string) (fv: VarSet.t)  : string =
  let newName = x ^ "0" in 
  if (VarSet.mem newName fv) then renaming newName fv else begin (print_endline "origName:"); (print_endline x); (print_endline "FV Set:"); (print_set fv); (print_endline "newName:"); (print_endline newName);newName end
  
    (* if (VarSet.mem newName fv) then renaming newName e else newName *)
...
let rec subst (x : string) (e1 : expr) (e2 : expr) : expr =
  ...
  | Lambda (u, t') -> if u != x then (if (VarSet.mem u (free_vars e1)) then let rename = renaming u (VarSet.union (free_vars e1) (free_vars e2)) in Lambda(rename,(subst x e1 (subst u (Var rename) t'))) else Lambda (u, (subst x e1 t'))) else e2
  | LetBind (u,t1,t2) -> if u != x then (if (VarSet.mem u (free_vars e1)) then let rename = renaming u (VarSet.union (free_vars e1) (free_vars e2)) in LetBind (rename, (subst x e1 (subst u (Var rename) t1)), (subst x e1 (subst u (Var rename) t2))) else LetBind (u, (subst x e1 t1), (subst x e1 t2))) else e2
